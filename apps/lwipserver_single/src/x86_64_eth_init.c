...
{
    ethif_intel_config_t *eth_config = NULL;
    int error = ps_calloc(&io_ops->malloc_ops, 1,
                          sizeof(ethif_intel_config_t) + sizeof(ps_irq_t),
                          (void **) &eth_config);
    if (error) {
        ZF_LOGF("ERROR allocate memory for ethernet");
        return error;
    }
    ...
    *eth_config = (ethif_intel_config_t) {
    ...
    eth_config->irq_info[0] = (ps_irq_t) { .type = PS_IOAPIC,
                                           .ioapic = { .ioapic = 0, .pin = 20,
                                                       .level = 1, .polarity = 1,
                                                       .vector = 20
                                                     }
                                         };
